...
| `LibKa0s-Widgets-1.0` | *(no consumers yet)* | Shipped at v1.11.0; the two intended adopters (BankLedger, whose `modules/Browser.lua` this widget was lifted from, and MultiMeters) have not re-vendored as of this release. Add each as its own row, with its wiring location, only once its own adoption commit actually lands — this table is maintained from the source, never pre-filled ahead of it |
