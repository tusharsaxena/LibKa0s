...
**Where v1.37.0 stands (2026-09-16).** Steps 1–9 are done. `master` carries the release at
`6f8e00c`, and the tag `v1.37.0` is pushed on the record commit `a7409f8`. Step 8 is done: all eleven
consumers bundle v1.37.0 and kit revision 21 on `master`, each pushed with its `CLAUDE.md` provenance
line moved in the same commit. Two adopt the release's one new surface, `MasterControls`'
`testModePath` (the standard's v2.46.0 Test mode row): PartyFrameEnhanced and AuraMaster. The other
nine re-vendored with no host change. The step-9 sweep found PartyFrameEnhanced, which has vendored
`libs/LibKa0s/` since its first commit, missing from both the Consumers table and the sweep loop;
both name it now, and its seven wired majors are in their rows. Move this paragraph at the next
release.
