...
|---|---|---|---|
| Repo semver (`v1.14.0`) | git tag, `CHANGELOG.md` heading | humans | once per release |
...
> Bundles [LibKa0s](https://github.com/tusharsaxena/LibKa0s) v1.14.0 (MIT).
...
| `LibKa0s-Widgets-1.0` | BankLedger, MultiMeters | BankLedger: `modules/Browser.lua`, which is where this widget was lifted FROM — its adoption at v1.11.1 is what found `CloseMenu()`. MultiMeters: `modules/Export.lua`, the second adopter and the reason the widget was lifted at all; it drives two dropdowns from one export modal and calls `CloseMenu()` on that modal's hide. Both look the major up once at file load and refuse to draw the surface at all when it answers `nil`. **Neither passes `glyphFont`** — the glyph column is BankLedger's store-direction case and nothing in either shipped host uses it today, which is why v1.11.0's and v1.11.1's crash on the first click reached both of them and was found by neither's suite |
