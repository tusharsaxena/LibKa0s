...
|---|---|---|---|
| Repo semver (`v1.31.0`) | git tag, `CHANGELOG.md` heading | humans | once per release |
...
1. **Make the change**, with its test. Green gate: `lua tests/run.lua` and `luacheck .` (0/0).
   ...
   is fifty-three files at v1.31.0: everything but `tests/_kit/`, which is excluded only because
