...
9. **Re-sweep the Consumers table against the source**, because it is maintained by hand and the
   ...
   ```
   for a in AbsorbTracker AuraMaster BankLedger ConsumableMaster KickCD LootHistory MultiMeters PanelMaster PrettyChat WhatGroup; do
...
|---|---|---|
| `LibKa0s-Core-1.0` | AbsorbTracker, KickCD, ConsumableMaster, BankLedger, LootHistory, MultiMeters, PanelMaster, PrettyChat, WhatGroup, AuraMaster | `core/CoreSetup.lua` (all ten). PrettyChat is the **first host to pass `sep = ""`** — its `[PC]` tag bakes its own trailing space, so the default `" "` would double-space every line it prints. It also **declines the window-chrome half** for the same reason PanelMaster does: its only window is the debug console, which reaches Core's chrome from inside the DebugLog major. PanelMaster **declines the window-chrome half** (`SKIN`/`ApplySkin`/`MakeCloseButton`): its only standalone window is the debug console, which reaches Core's chrome from inside the DebugLog major |
| `LibKa0s-DebugLog-1.0` | AbsorbTracker, KickCD, ConsumableMaster, BankLedger, LootHistory, MultiMeters, PanelMaster, PrettyChat, WhatGroup, AuraMaster | `core/DebugLogSetup.lua` (all ten) — ConsumableMaster's was at `modules/DebugLog.lua` when this table was written and is not there now; that file no longer exists, and the lookup is at `core/DebugLogSetup.lua:59` with everyone else's. PrettyChat passes **none** of `skin` / `applySkin` / `makeCloseButton` and deleted a 424-line hand-written console to take the library's edge as-is — the first adoption where the Core-minor-3 default *was* the answer rather than something to override. **LootHistory is the second host on minor 4's `applySkin` — and both it and BankLedger have since
...
**Where v1.30.0 stands in the consumers (2026-09-12).** Step 8 is done but not merged. All ten
consumers carry the re-vendor on an unmerged branch, `chore/libka0s-1.30.0-arch5`, and on that branch
the `CLAUDE.md` provenance line reads v1.30.0. Each `master` still bundles v1.29.0 and says so. Since
no file in `LibKa0s/` moved at v1.30.0, the only thing a consumer's `master` is missing is kit
revision 16 in `tests/_kit/`, plus the local shims that revision retires. The step-9 sweep above was
run against those branches. Their lookup sites match each `master` site for site, so the table holds
before and after the merge. Rewrite this paragraph when the branches land. `WhoGotLoots` and `BuffTextNotifications`
