...
| `LibKa0s-Lifecycle-1.0` | **none yet** — the major is unreleased, and until the tag that ships it exists every addon's adoption is **blocked** rather than overdue: the standard cites a tag number, and a number invented ahead of the release is a citation that does not resolve. All eleven adopt it, because `LibKa0s-Perf-1.0` minor 12 requires it and the disabled-state rule is built on the same latch. The wiring will live beside `core/PerfSetup.lua`, which already calls the host's inert routine and is the seam this major generalizes. **Do not let this cell read "none yet" a release after the first host wires it** — the Media, Env and Launcher rows all did exactly that. |
