...
| `LibKa0s-Lifecycle-1.0` | **none yet** — the major ships at v1.40.0 and every consumer's adoption is `slash-commands-§7`'s changeset, not this release's. The tag that ships it now exists, so every addon's adoption is **overdue** rather than blocked — the standard cites a tag number, and until v1.40.0 there was none to cite. All eleven adopt it, because `LibKa0s-Perf-1.0` minor 12 requires it and the disabled-state rule is built on the same latch. The wiring will live beside `core/PerfSetup.lua`, which already calls the host's inert routine and is the seam this major generalizes. **Do not let this cell read "none yet" a release after the first host wires it** — the Media, Env and Launcher rows all did exactly that. |
...
- `LibKa0s-Perf-1.0` minor 12, whose suspend arm moves onto that latch and whose floor now names
...
**v1.39.0's step 8 IS done**, and this paragraph's predecessor said it was not: all eleven consumers'
`CLAUDE.md` provenance lines read v1.39.0. What is still outstanding from that release is the
`launcher-§5` **adoption** changeset, which was never the re-vendor — vendor LibDataBroker-1.1 and
LibDBIcon-1.0, generate the 128×128 logo, build the object, declare `minimapPath`, record the rung in
`ADDONS.md`, with Multi Meters additionally owing a `db.profile.minimap` → `db.global.minimap`
