...
|---|---|---|---|
| Repo semver (`v1.60.0`) | git tag, `CHANGELOG.md` heading | humans | once per release |
| File minor (integer) | `MINOR` / `DRAG_MINOR` / `DIAG_MINOR` / `WIDGETS_MINOR` / `TABS_MINOR` / `SCROLL_MINOR` / `COMPOSE_MINOR` / `PANEL_MINOR` at the top of each file in `LibKa0s/` | **LibStub, at load time** | every released change to that file |
...
2. **Bump the minor of every file you changed** — and if you touched `testkit/`, bump
   ...
   twenty-two, by their exact constant names: `MINOR` in `Core.lua`, `MINOR` in `Env.lua`, `MINOR` in
...
**The library** is the inner `LibKa0s/` folder and nothing else — the twenty-two `.lua` files, the
...
**Where v1.60.0 stands (2026-09-26).** Three LibStub minors move and one file is added:
