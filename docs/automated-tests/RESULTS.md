...
| 1000–1500 (on notice) | `tests/test_options_idsuggest.lua` | 1002 | **Accepted 2026-09-24 (`LK-33`), re-read 2026-09-26 (`LK-ATS-01`).** 999 → 1002 when `LK-05`'s shown-by-default frames gave one case the `Hide()` production performs. #32's peel, one of its two triggers, has fired: the id-suggestion half it mirrors is now `LibKa0s/OptionsIds.lua`, so it pairs with that module (`testing-§1`) and moves no case. **Re-check trigger: 1200 lines.** |
