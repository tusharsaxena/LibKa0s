...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| [`20260922-202214`](20260922-202214/) | 1.53.0 → 1.54.0 | 0/0 | 73 | 1277/0/1277 | skip | 25542 | 3594 | 6.6 | 2.0 | 15 | 0 | **green** |
| [`20260922-170122`](20260922-170122/) | 1.52.0 → 1.53.0 | 0/0 | 72 | 1277/0/1277 | skip | 25362 | 3590 | 6.6 | 2.0 | 15 | 0 | **green** |
| [`20260922-095841`](20260922-095841/) | 1.51.0 → 1.52.0 | 0/0 | 72 | 1276/0/1276 | skip | 25354 | 3589 | 6.6 | 2.0 | 15 | 0 | **green** |
| [`20260922-022127`](20260922-022127/) | 1.50.0 → 1.51.0 | 0/0 | 72 | 1270/0/1270 | skip | 25262 | 3580 | 6.6 | 2.0 | 15 | 0 | **green** |
...
**1277 cases** — 1277 passed, 0 failed, 0 skipped. The generated inventory
[`20260922-202214/test-cases.md`](20260922-202214/test-cases.md) is the authority on which cases existed at this run;
`docs/test-cases.md` is that same list at HEAD.

Unchanged from the previous run at 1277 cases.
...
**0 warnings / 0 errors over 73 files** (`luacheck .`).
...
Current as of [`20260922-202214`](20260922-202214/) — **this run's measurement, not its diff.** Max CCN **15** across 3594
...
| > 1500 (over cap) | `LibKa0s/OptionsWidgets.lua` | 3922 | **Ruled on 2026-09-08 (`M4-14`), tracked as [`#16`](https://github.com/tusharsaxena/LibKa0s/issues/16)** (owner: @tusharsaxena). A breach, not a band entry: `layout-§1` was revised this cycle to say the cap binds a library's own payload folder, which is the question the 2026-09-07 audit graded Low for want of an answer. The issue names the seam — the tab and page chrome (`:378` art block, `:871`-`:1349` members) out to `OptionsTabs.lua`, leaving the widgets and the flow engine; the two halves share no local in either direction. Not peeled this cycle: the remediation plan forbids splitting, and a payload peel here costs an `LibKa0s.xml` row, a LibStub minor and the pairing guard. `CLAUDE.md` § *Files over the 1500-line cap* is the census and `tests/test_layout_cap.lua` reddens if this file ever drops off it. |
| > 1500 (over cap) | `tests/test_options_widgets.lua` | 4086 | **Ruled on 2026-09-08 (`M4-14`), tracked as [`#8`](https://github.com/tusharsaxena/LibKa0s/issues/8)** (owner: @tusharsaxena). That issue's own hard trigger — *crosses 1500 → split* — has fired; it was rewritten from an on-notice record (1114) into the breach record it now is. The seam changed with it: **not** by widget family, but along whatever seam `LibKa0s/OptionsWidgets.lua` peels on ([`#16`](https://github.com/tusharsaxena/LibKa0s/issues/16)), in that file's own commit — `testing-§1` pairs one suite with one module, and a suite peeled on a partition the module has not chosen leaves two files that no longer pair. |
