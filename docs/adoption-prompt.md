...
Slash, **declining Perf** on two independent structural grounds (`LIBKA0S-15`): it has no hot path
at all, and `suspend` would stop a CAPTURE addon capturing, so a measurement window would cost the
user the group notification they joined for. It is also the first adopter to drive a change to a
surface every host already used — see **DebugLog minor 7** below.
