...
Remaining targets: `prettychat`, `WhatGroup` — and `LootHistory`, whose `libs/LibKa0s/` exists and
whose rows are already in `docs/releasing.md`'s Consumers table; whoever ran it did not update this
line, so treat its entry below as unverified rather than as pending work.
