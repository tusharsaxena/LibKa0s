# `LibKa0s-DebugLog-1.0` — version 4
...
| Files and minors | `DebugLog.lua` minor **4** |
| Shipped in | v1.2.0 |
| Status | Superseded |
| Supersedes | minor 3 — [version 3](./version-3-docs.md) |
| Superseded by | [version 5](./version-5-docs.md) |
| Requires | `LibKa0s-Core-1.0` minor ≥ 1 (`NEEDS_CORE = 1`) |
| Confirm in-game | `LibStub("LibKa0s-DebugLog-1.0").MODULES` → `{ DebugLog = 4 }` |
...
**Two new descriptor fields, both optional and both additive**: `applySkin` and
`makeCloseButton`. Together they let a host own its console's window chrome — `skin` is a table and
therefore reaches only the three backdrop calls, so a host whose chrome also has a synthesised
inner-border child frame, a title tint or a divider tint needs *calls*, not fields. Nothing existing
changes; a host that passes neither gets exactly what minor 3 gave it.
