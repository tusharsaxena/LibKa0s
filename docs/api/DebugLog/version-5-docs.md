# `LibKa0s-DebugLog-1.0` — version 5
...
| Files and minors | `DebugLog.lua` minor **5** |
| Shipped in | v1.3.0 |
| Status | Superseded |
...
| Confirm in-game | `LibStub("LibKa0s-DebugLog-1.0").MODULES` → `{ DebugLog = 5 }` |
...
**A look change, not an API change — nothing to do at a call site.** This minor rides Core minor
3, whose `lib.SKIN` moved to the flat 1px Ka0s edge and gained `innerBorder`, `divider` and `title`.
The library's own default now draws that full edge for the console and the copy window, so a host
that had passed `applySkin` purely to get a matching border no longer needs to. The `skin` field is
also documented as degrading rather than raising on a partial table.
