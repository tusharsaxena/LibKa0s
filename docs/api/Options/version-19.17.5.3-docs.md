...
| Superseded by | [version 19.18.5.3](./version-19.18.5.3-docs.md) |
...
## Moving to version 19.18.5.3

One file moves, `OptionsWidgets.lua` 17 → 18: a defect fix, nothing added, removed, renamed or
resignatured. `O.ChoiceGrid`'s cell fill leaked a gold tint into the next `CheckBox` AceGUI handed
out from its pool, because `choiceFill` never restored the check texture's vertex color on
`OnRelease`. From 18 it does. Every host that draws a `ChoiceGrid` is affected whether or not it
uses `extraColumn` or `note` — the re-vendor is the whole fix. See
[version 19.18.5.3](./version-19.18.5.3-docs.md).
