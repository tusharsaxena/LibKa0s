# `LibKa0s-Options-1.0` — version 4.4.2
...
| Files and minors | `Options.lua` **4** · `OptionsWidgets.lua` **4** · `OptionsScroll.lua` **2** |
...
| Status | Superseded |
| Supersedes | [version 3.3.2](./version-3.3.2-docs.md) |
| Superseded by | [version 5.5.2](./version-5.5.2-docs.md) |
| Requires | `LibKa0s-Core-1.0` minor ≥ 1 (`NEEDS_CORE = 1`) |
| Confirm in-game | `LibStub("LibKa0s-Options-1.0").MODULES` → `{ Options = 4, OptionsWidgets = 4, OptionsScroll = 2 }` |
...
- **`Options.lua` minor 4** — every row is guarded individually while a page renders, so one bad row
  costs a row rather than the page.
- **`OptionsWidgets.lua` minor 4** — `RenderGrid` arrives, for a list whose length is not in the
  schema; and `LSMValues` is never empty, yielding a single `None` placeholder when the media
  library has not loaded, because a dropdown with no options cannot be opened and the CLI would
  refuse even the stored value.
...
| Name | Since | Meaning |
|---|---|---|
| `CreatePanel(name, title, opts)` | O1 | A canvas Frame with the unified header stamped on top, returning the `ctx` every render call threads through. `opts` = `{ pageKey, isMain, defaultsButton, defaultsTooltip }`. Registers the ctx so the refresh fan-out reaches it. |
...
| `RenderGrid(ctx, items)` | **W4** | Lay arbitrary widgets out two per row, caller-ordered. The sibling of `RenderRows`: that one walks schema rows and emits sections, this one takes whatever the caller hands it — a schema row, or `{ make = fn }` for a bespoke widget, or `wide = true` for its own line. For a list whose length is not in the schema (one checkbox per macro, per unit, per spell). Items are guarded individually. |
...
## Moving to version 5.5.2

Nothing to change at a call site. `CreatePanel` starts stamping the Blizzard canvas contract, and a
`number` row that declares `values` starts rendering as a dropdown instead of a slider — which is a
*rendering* change for a schema shape `LibKa0s-Slash-1.0` already parsed that way. Check any
`number` row of yours that carries a `values` list.
