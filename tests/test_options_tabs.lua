...
-- with nothing.
--
-- WHAT IS HERE. Every case whose subject is chrome. `O.RenderTabbedSchema` is NOT chrome — it is
-- the flow engine's tabbed entry point and it stays in OptionsWidgets.lua — so the cases under
-- *the tabbed page* stayed with it, which is the one place this cut differs from the banner list
-- issue #8 wrote down. They read a strip because that is what the entry point draws; what they
-- assert is which ROWS a tab shows, and a red there sends you to the flow engine.
--
