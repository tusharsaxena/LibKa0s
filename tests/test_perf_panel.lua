...
test("lib: finishing unlocks Report", function()
  local p = Fixture.new()
  p.Start("panel")
  p.Measure("a"); tick(p, 0.5, true); tick(p, 0.5, false)
  p.Measure("b"); tick(p, 0.5, true); tick(p, 0.5, false)
  ...
  -- One review step, not two: report carries the JSON now, so a `dump` field reappearing here
  -- means the fold was undone.
  assertEqual(s.dump, nil, "dump is a step again")
end)
...
test("lib: report stays clickable after use, but reads as done", function()
...
test("lib: marking a review action twice is a no-op", function()
  ...
  assertFalse(p.MarkReviewed("report"), "second time changes nothing")
  p.Stop()
end)
...
test("lib: every step label names what it acts on", function()
  ...
  assertEqual(f.buttons.report.__label, "Report", "report")
