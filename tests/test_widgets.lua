...
-- reason.
--
-- The factory is installed by these suites (tests/fixture_geom.lua holds it, shared with
-- tests/test_widgets_reorderlist.lua) rather than in tests/wow_mock.lua deliberately: fifteen other
...
-- on its first open, and once per case, by the `test` tests/fixture_geom.lua hands back, which swaps it in around the
...
-- The geometry factory and the per-case `test` that installs it live in tests/fixture_geom.lua,
-- shared with tests/test_widgets_reorderlist.lua since issue #37 moved the ReorderList cases there.
local Geom = dofile("tests/fixture_geom.lua")
local geomCreateFrame, geomUIParent, test = Geom.createFrame, Geom.uiParent, Geom.test
...
-- and `test` is the geometry-installing wrapper loaded at the head, so both are used directly.
