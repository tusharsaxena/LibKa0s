...
test("options: a host mutes its seam's [Set] inside the bracket and logs ONE line — the documented "
  ...
  -- The shape docs/api/Options/version-16.15.4.3-docs.md gives, run for real: a write seam that logs
  -- one `[Set]` per write, a bulk depth that mutes it, and one summary line on the way out.
  local log, bulkDepth = {}, 0
  local store = {}
  local function Set(path, value)                -- the host's single write seam
    store[path] = value
    if bulkDepth == 0 then log[#log + 1] = ("[Set] %s = %s"):format(path, tostring(value)) end
  end
  local O, rec = Fixture.new{
    set          = function(path, v) Set(path, v) end,
    bulkBegin    = function() bulkDepth = bulkDepth + 1 end,
    bulkEnd      = function(act, scope, count)
      bulkDepth = bulkDepth - 1
      log[#log + 1] = ("[Set] %s %s page: %d rows"):format(act, tostring(scope), count)
    end,
  }
  rec.d.applyDefault = function(row) Set(row.path, row.default) end

  O.RestoreDefaults("bar")
  local n = #rec.d.rowsForPage("bar")
  assertEqual(#log, 1, "one line for the whole reset: " .. table.concat(log, " | "))
  assertEqual(log[1], ("[Set] reset bar page: %d rows"):format(n))
  assertEqual(bulkDepth, 0, "the mute is released")
  assertEqual(store.barWidth, 200, "and every row still went through the seam")

  rec.d.set("barWidth", 250)                     -- a plain write after the reset
  assertEqual(log[2], "[Set] barWidth = 250", "and the seam logs per write again")
end)
...
test("options: with NO bracket the walk is exactly minor 15's — same calls, same order, and an "
  ...
  assertFalse(ok)
  assertTrue(tb:find("test_options.lua:" .. raiseLine .. ": row down", 1, true) ~= nil,
    "the message is the row's own: " .. tb)
  assertTrue(select(2, tb:gsub("test_options.lua:" .. raiseLine .. ":", "")) >= 2,
