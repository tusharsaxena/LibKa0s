...
the standard's text and ends when the owner's ruling lands either way. The sixth keeps three
widget-owned UI frames on raw core-event registration, outside a rule written for addon event
traffic. The table is otherwise empty
on purpose:
