...
different vendored copy of each. As of **v1.26.0**: `Core = { Core = 7 }`,
`Env = { Env = 1 }`, `Pool = { Pool = 3 }`, `Item = { Item = 1 }`, `Media = { Media = 3 }`,
`Widgets = { Widgets = 9 }`, `DebugLog = { DebugLog = 12 }`, `Slash = { Slash = 7 }`,
`Options = { Options = 14, OptionsWidgets = 14, OptionsCompose = 3, OptionsScroll = 3 }`,
