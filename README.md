...
different vendored copy of each. As of **v1.32.0**: `Core = { Core = 7 }`,
`Env = { Env = 1 }`, `Pool = { Pool = 3 }`, `Item = { Item = 1 }`, `Media = { Media = 3 }`,
`Widgets = { Widgets = 9 }`, `DebugLog = { DebugLog = 12 }`, `Slash = { Slash = 8 }`,
`Options = { Options = 16, OptionsWidgets = 15, OptionsCompose = 4, OptionsScroll = 3 }`,
