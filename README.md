...
Built to the **[Ka0s WoW Addon Standard](https://github.com/tusharsaxena/WowAddonStandards)**, v2.59.1
...
different vendored copy of each. As of **v1.45.0**: `Core = { Core = 7 }`,
`Env = { Env = 1 }`, `Lifecycle = { Lifecycle = 1 }`, `Pool = { Pool = 3 }`, `Item = { Item = 1 }`,
`Media = { Media = 3 }`,
`Widgets = { Widgets = 9 }`, `DebugLog = { DebugLog = 12 }`, `Slash = { Slash = 14 }`,
`Launcher = { Launcher = 1 }`,
`Options = { Options = 21, OptionsWidgets = 22, OptionsTabs = 1, OptionsCompose = 7, OptionsScroll = 3 }`,
