...
- **`LibKa0s-Options-1.0`** — the Blizzard settings-canvas shell, the schema-row to AceGUI widget
  ...
  declaration into a canonical block. Ten files, one major.
...
1. Copy `LibKa0s/` into `<Addon>/libs/LibKa0s/` — the whole folder, every time. The modules are
   ...
   registering at all when `Core.lua` is missing or older than the minor it needs. When `Options.lua` bails that way, `OptionsRegistry.lua`, `OptionsWidgets.lua`, `OptionsIds.lua`, `OptionsIdList.lua`,
   `OptionsTabs.lua`, `OptionsCombat.lua`, `OptionsCompose.lua`, `OptionsScroll.lua` and `OptionsNav.lua` bail too on their own
   `LibStub("LibKa0s-Options-1.0", true)` lookup, so the whole ten-file module is absent rather
